...
public class BagelTest {
    ...
    public void addFillingsTest() {
        ...
        for(int i = 0; i < bagel.getFillings().size(); i++) {
            Assertions.assertEquals(testFillings[i].getName(), bagel.getFillings().get(i).getName());
            Assertions.assertEquals(testFillings[i].getPrice(), bagel.getFillings().get(i).getPrice());
        }
